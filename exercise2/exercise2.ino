// Exercise 2: joystick-based differential drive control with analog speed and turn mixing.
#include <Arduino.h>
...
MotorCommand computeMotorCommand(int rawX, int rawY) {
  int turnPercent = normalizeAxis(rawX);  // Negative = left turn, positive = right turn
  int speedPercent = normalizeAxis(rawY); // Positive = forward, negative = backward
  ...
  command.leftPwm = speedPercentToPwm(abs(leftPercent));
  Serial.println(leftPercent);
  command.rightPwm = speedPercentToPwm(abs(rightPercent));
